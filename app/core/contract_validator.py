    ...
    for index, interaction in enumerate(contract['http_interactions']):
        ...
        if path not in spec['paths']:
            raise ValidationException(
                f'Given inappropriate path "{path}" in interaction[{index}]'
            )
        ...
        if status_code not in spec['paths'][path][method]['responses']:
            raise ValidationException(
                f'Requested path "{path}" does not support  method "{method}"'
            )
        ...
        if contract_payload:
            if (
                content_type
                not in spec['paths'][path][method]['responses'][status_code]['content']
            ):
                # TODO
            ...
            for content in spec['paths'][path][method]['responses'][status_code][
                'content'
            ]:
                if not content:
